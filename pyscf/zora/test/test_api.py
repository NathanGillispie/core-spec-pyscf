...
def test_spin_orbit_ignored_on_rhf():
    mol = make_hf_mol()
    mf = pyscf.scf.RHF(mol).zora(spin_orbit=True, grid_level=TEST_GRID_LEVEL)
...
def test_custom_grid_matches_grid_level():
    ...
    h_level = pyscf.scf.RHF(mol).zora(grid_level=TEST_GRID_LEVEL).get_hcore()
...
def test_to_ks_keeps_zora():
    ...
    assert ks.with_zora.grid_level == TEST_GRID_LEVEL
...
def test_generalized_gradients_require_pyscf_support(monkeypatch):
    ...
    mf = pyscf.scf.GHF(make_hf_mol()).zora(grid_level=TEST_GRID_LEVEL)
...
def test_idempotent_updates_options():
    ...
    h3 = mf.get_hcore()
    assert mf.with_zora.grid_level == TEST_GRID_LEVEL
...
def test_mo_energy_correction_is_default_and_can_be_disabled():
    ...
    mf_no_corr = pyscf.scf.RHF(mol).zora(
        grid_level=TEST_GRID_LEVEL, mo_energy_correction=False)
...
def test_uhf_mo_energy_correction_changes_occupied_energies():
    mol = make_hf_mol()
    mf = pyscf.scf.UHF(mol).zora(grid_level=TEST_GRID_LEVEL)
...
def test_ghf_mo_energy_correction_changes_occupied_energies():
    mol = make_hf_mol()
    mf = pyscf.scf.GHF(mol).zora(grid_level=TEST_GRID_LEVEL)
